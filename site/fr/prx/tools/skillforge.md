...
Skillforge est le pipeline automatise pour decouvrir, evaluer et integrer de nouvelles competences (tools) from external sources. Instead of manually configuring every tool, Skillforge can scout GitHub repositories and the Clawhub registry, evaluate whether a discovered skill fits your agent's needs, and generate the integration manifest -- all without human intervention.
...
| **Integrate** | `Integrator` | Generate manifests and register skills into le registre d'outils |

## Architecture

Skillforge est construit sur trois traits async principaux: `Scout` (decouvre les candidats correspondant a `SearchCriteria`), `Evaluator` (scores candidates for fitness and security), and `Integrator` (generates manifests and registers skills). Each trait can have multiple implementations, and le pipeline orchestrator runs them in sequence, filtering candidates at each stage.
...
Le Scout decouvre les competences candidates depuis les sources configurees. Chaque source implemente le trait `Scout` differently:
...
Queries the Clawhub registry API for published skills. Clawhub provides structured metadata incluant :
...
Chaque candidat passe par l'Evaluateur, qui produit un score de fitness et une evaluation de securite:
...
The `Evaluation` struct contient le overall `fitness_score` (0.0-1.0), per-criterion scores, a `security_status` (`safe`/`caution`/`blocked`), a human-readable summary, and a list of concerns.
...
The Integrator genere a `Manifest` that describes how to install and register the skill:
...
Once the manifest is generated, the skill est enregistre in the PRX registre d'outils. If `require_approval = true`, the manifest is staged for review:
...
Skillforge integrates with PRX's [self-evolution pipeline](/fr/prx/self-evolution/). When l'agent identifies a capability gap, it can trigger a discovery run automatically -- scouting, evaluating, and (if approved) integrating a matching skill pour le next tour.

## Securite Nontes

- **Approval gates** -- always set `require_approval = true` in production. Automated integration of untrusted code is a security risk.
- **Sandbox enforcement** -- integrated skills run within the same sandbox constraints as outils integres. Le sandbox backend doit etre configured.
- **Source trust** -- only enable scout sources that you trust. Public GitHub search can retour malicious repositories.
- **Manifest review** -- review generated manifests before approval. Check the `runtime`, `network`, and `timeout_secs` settings.
- **Audit trail** -- all Skillforge operations sont journalises in the activity log for compliance review.

## Voir aussi Pages

- [Tools Overview](/fr/prx/tools/)
- [Self-Evolution Pipeline](/fr/prx/self-evolution/pipeline)
- [Security Moteur de politiques](/fr/prx/security/policy-engine)
- [Runtime Backends](/fr/prx/agent/runtime-backends)
- [MCP Integration](/fr/prx/tools/mcp)
